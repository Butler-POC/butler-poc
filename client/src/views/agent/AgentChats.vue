<!-- A-02 건물주 연결 (에이전트측) — 임대인과의 대화 목록 -->
<!-- 공실 조회(A-01)에서 시작한 임대인 대화를 목록으로 보여주고, 선택 시 채팅. -->
<!-- 룸키 prefix 'vacancy:' 로 공실 대화 필터. (가정: useChatStore.threads / loadThreads()) -->
  ...
  <section class="page">
    ...
    <template v-if="activeRoom">
      <header class="bar">
        ...
        <h1 class="bar__title">임대인</h1>
      </header>
      <div class="page__body">
        <VacancyChat :room-id="activeRoom" counterpart-name="임대인" />
      </div>
    </template>
    ...
    <template v-else>
      ...
      <header class="head">
        <p class="eyebrow">건물주 연결</p>
        <h1 class="title">임대인 대화</h1>
      </header>

      <p v-if="threads.length === 0" class="empty">
        <span class="empty__dot" aria-hidden="true" />진행 중인 대화가 없습니다. 공실 조회에서 임대인에게 먼저 말을 걸어보세요.
      </p>
